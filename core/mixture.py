...
import math
import copy
from core.pymix_util.alphabet import Alphabet,IntegerRange
...
class DataSet(object):
    ...
    def fromFile(self, fileName, sep = '\t', col_headers=False, row_headers=False):
        ...
        # checking data-label consistency
        for i in range(self.N):
           assert len(self.dataMatrix[i]) == len(self.headers), "Different numbers of headers and data columns in files "+ str(fileName) + ", sample " + str(self.sampleIDs[i]) + " ," + str(len(self.dataMatrix[i])) + " != " + str( len(self.headers))
        ...
        if isinstance(data,DataSet):
            ...
            for i in range(self.dist_nr):
                if isinstance(self.distList[i], MixtureModel): # XXX only necessary for mixtures of mixtures
                    ...
                    #print i, len(data.getInternalFeature(i)) , len(data.getInternalFeature(i)[0])
