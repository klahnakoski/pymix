...
from pymix.util.maths import dict_intersection
from pymix.util.stats import sym_kl_dist, get_loglikelihood, random_vector
...
#        #print '*** seed=',seed
    ...
    sigma_ = UniformDistribution(0.1, 1.0)
    mu_ = UniformDistribution(-5.0, 8.0)
...
#        #print '*** seed=',seed
    ...
    sigma_ = UniformDistribution(0.3, 5.0)
    mu_ = UniformDistribution(-25.0, 25.0)
...
def scoreStructureLearning_diffFullVsTopdown(N, gen, delta, seed=None, silent=False, skipAfterRNGcalls=False):
    ...
    elif str(compred) != '[]' and nr_full_lead > m4.dimension and match != 1:  # redundant components and not fully merged
        print '-----------------------------------------------------------------------'
