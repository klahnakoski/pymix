...
def ghmm_dmodel_viterbi(mo, o, len, pathlen, log_p)
:
#define CUR_PROC "ghmm_dmodel_viterbi"
    ...
    for t in range( 1,  len):
        ...
        for St in range(0,  mo.N):
            # Determine the maximum 
            # max_phi = phi[i] + log_in_a[j][i] ... 
            if not(mo.model_type & GHMM_kSilentStates) or notmo.silent[St]:
                max_value = -DBL_MAX
                max_id = -1
                for i in range( 0,  mo.s[St].in_states):
                    i_id = mo.s[St].in_id[i]
    ...
    if end_state < 0:
        # Sequence can't be generated from the modelnot 
        *log_p = +1
        len_path = 1
    ...
    for ( state_seq_index >= 0  state_seq_index--) :
        ...
        if not(mo.model_type & GHMM_kSilentStates) or notmo.silent[next_state]:
            t--
    
    *pathlen = len_path
    if state_seq_index >= 0 or t > 0:
